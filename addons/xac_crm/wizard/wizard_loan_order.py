...
def _open_window(self, cr, uid, data, context):
    if data.get('id', False):
         ...
         cr.execute('select id,name from ir_ui_view where name=%s and type=%s', ('view.loan.order.form', 'form'))
         view_form = cr.fetchone()[0]
         cr.execute('select id,name from ir_ui_view where name=%s and type=%s', ('view.loan.order.tree', 'tree'))
         ...
         res = {
                    'name': 'Зээлийн өргөдөл бүртгэх - %s '%(ref),
                    'view_type': 'form',
                    "view_mode": 'tree,form',
...
class wizard_loan_order(wizard.interface):
    states = {
        'init': {
            'actions': [],
...
wizard_loan_order('wizard.loan.order')
